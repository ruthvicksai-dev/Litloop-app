...
export const getDashboardAnalytics = query({
  ...
  handler: async (ctx, args) => {
    ...
    const [dailyStats, monthlyStats, topBookStats, topGenreStats] =
      await Promise.all([
        ctx.db
          .query("analytics_daily")
          .withIndex("by_date")
          .filter((q: any) =>
            q.or(...dateKeys.map((d) => q.eq(q.field("date"), d))),
          )
          .collect(),
        ctx.db
          ...
          .withIndex("by_month")
          .filter((q: any) =>
            q.or(...requestedMonthKeys.map((m) => q.eq(q.field("month"), m))),
          )
          .collect(),
        ctx.db
          .query("book_stats")
    ...
    const [fallbackTotalUsers, fallbackTotalRentals, fallbackActiveRentals] =
      await Promise.all([
        countersReady !== 1
          ? ctx.db
              .query("users")
    ...
    const totalRentals =
      countersReady === 1
        ? (totalRentalsCounter ?? 0)
        : (fallbackTotalRentals ?? 0);
    const activeRentalsCount =
      countersReady === 1
        ? (activeRentalsCounter ?? 0)
        : (fallbackActiveRentals ?? 0);
    ...
    return {
      ...
      leaderboards: {
        topRentedBooks,
        topRevenueBooks: [...topRentedBooks].sort(
